...
/*		ADODB_Active_Record::__construct($table,$pkeyarr,$db);
	...
	function Set(&$row, $join = false)
	{
		...
		if (!$join && sizeof($table->flds) != sizeof($row)) {
	...
	function Load($where,$bindarr=false, $joins = false)
	{
		...
		if (is_array($joins))
		{
			...
			{
				$is_only = true;
				$join .= $this->_createJoin($joins);
			} else {
				$is_only = false;
				foreach($joins as $j)
				{
					$join .= $this->_createJoin($j) . ' ';
				}
			}
		}       
		$save = $db->SetFetchMode(ADODB_FETCH_NUM);
		$row = $db->GetRow("select ".$this->_table.".* from ".$this->_table.' '.$join.' WHERE '.$where,$bindarr);

		$db->SetFetchMode($save);

		$return_set = $this->Set($row, $is_join);

		if ($is_join && $is_only)
		{
			$table = $joins['table'];
			$this->$table = array_values($this->_join);
		}

		return $return_set;
	}

	function _createJoin($join)
	{
        if (empty($join['type']))
        {
            $join['type'] = 'INNER';
        }
        if (empty($join['father']))
        {
            $join['father'] = $this->_table;
        }
		if (!empty($join['field']))
		{
			$childField = $fatherField = $join['field'];
		} else {
			$childField  = $join['childField'];
			$fatherField = $join['fatherField'];
		}
		return $join['type']. ' join ' . $join['table'] . ' on ' . $join['table'] . '.' . $childField . ' = ' . $join['father'] . '.' . $fatherField . ' ' . $join['extendedJoin'];
	}
	// false on error
	function Save()
	{
	...
	function &Find($whereOrderBy,$bindarr=false,$pkeysArr=false, $joins = false)
	{
        $join = '';
        if (is_array($joins))
        {
		...
		$arr =& $db->GetActiveRecordsClass(get_class($this),$this->_table, $whereOrderBy,$bindarr,$pkeysArr, $join);
		return $arr;
	}
