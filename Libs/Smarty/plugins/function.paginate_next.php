...
function smarty_function_paginate_next($params, &$smarty) {
    ...
    if (!is_object($smarty->paginate->$_id)) {
        $smarty->trigger_error("paginate_next: paginate not initialized");
        return;
    }
    ...
    if(($_item = $smarty->paginate->$_id->_getNextPageItem()) !== false) {
        $_show = true;
        $_text = isset($params['text']) ? $params['text'] : 'next';
