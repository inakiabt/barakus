...
function smarty_function_pagination_middle($params, &$smarty) {

    $_id = empty($params['id']) ? 'p' : $params['id'];
    $tpl  = $GLOBALS['pagination_config'][$_id]['tpl'];
    $selected = $GLOBALS['pagination_config'][$_id]['selected'];
    ...
    if (is_object($layout))
    {
        ...
        if ($pager->haveToPaginate())
        {
            ...
            $layout->setSelectedTemplate($selected);
            $layout->removeMaskReplacement('page');
