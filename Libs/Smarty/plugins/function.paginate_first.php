...
function smarty_function_paginate_first($params, &$smarty) {
    ...
    if (!is_object($smarty->paginate->$_id)) {
        $smarty->trigger_error("paginate_first: paginate not initialized");
        return;
    }
    ...
    $_text = isset($params['text']) ? $params['text'] : 'first';
    $_url .= (strpos($_url, '?') === false) ? '?' : '&';
    $_url .= $smarty->paginate->$_id->getUrlVar() . '=1';
    $_url .= '&pagids=' . $_id . (empty($params['anchor']) ? '' : '#' . $params['anchor']);
    
    return '<a href="' . $smarty->url($_url) . '" title="1"' . $_attrs . '>' . $_text . '</a>';
}
