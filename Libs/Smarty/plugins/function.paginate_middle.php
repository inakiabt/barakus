...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    if (!is_object($smarty->paginate->$_id)) {
        $smarty->trigger_error("paginate_middle: paginate not initialized");
        return;
    }
    ...
    if(!isset($_page_limit) && $smarty->paginate->$_id->getLimit() > 0) {
        $_page_limit = $smarty->paginate->$_id->getLimit();
    }
    ...
    $_curr_item = $smarty->paginate->$_id->getCurrentItem();
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
            ...
            $_this_url .= $smarty->paginate->$_id->getUrlVar() . '=' . $_item;
            $_this_url .= '&pagids=' . $_id . (empty($params['anchor']) ? '' : '#' . $params['anchor']);
            $_ret .= $_link_prefix . '<a href="' . $smarty->url($_this_url) . '" title="'.$_item.'" ' . $_attrs . '>' . $_text . '</a>' . $_link_suffix;
        } else {
